...
class TeamScraper:
    ...
    def _validate_date_range(self, st, ed):
        ...
        if ed.year > dt.datetime.now().year+1:
    ...
    def _url(self):
        return "http://www.basketball-reference.com/" + \
            "teams/{}/{}_games.html".format(self.team,
    ...
    def _make_numeric(self, data):
        ...
        if data['Attend.'].count() > 0:
            data['Attend.'] = data['Attend.'].str.replace(',', '')
        else:
            data['Attend.'] = np.nan

        # Replace unknown with NaN so that column can be numeric
        data['Attend.'].replace(r'^Unknown$', np.nan, regex=True,
                                   inplace=True)

        # now make everything numeric
        num_cols = ["Tm", "Opp", "W", "L", "Attend."]
    ...
    def _process_date(self, data):
        ...
        def helper(val):
            ...
            if s >= 0:
                val = val[0:s]
            dv = dt.datetime.strptime(val, '%a, %b %d, %Y')
...
class TeamSummaryScraper:
    """A scraper that retrieves a summary of all of the basketball teams

    >>> from basketball_scraper import basketball_reference
    >>> tss = basketball_reference.TeamSummaryScraper()
    >>> df = tss.scrape(2019)
    >>> df.columns
    ...
    def scrape(self, year):
        """Scrape basketball-reference.com to get a list of all teams
    ...
    def _url(self, year):
        return "https://www.basketball-reference.com/leagues/MLB/{}.shtml" \
            .format(year)
